...
export function initGlobe(container, data, onSelect) {
  ...
  function drawBursts(t) {
    ...
    const M = 300 * dpr; // off-viewport margin: beyond this, a star can't reach in
    ...
    for (const p of participantPoints) {
      ...
      if (p.lifted && leadersVisible) {
        ...
        const lx = la.x * dpr, ly = la.y * dpr, sx = sc.x * dpr, sy = sc.y * dpr;
        // At close zoom the raised anchor projects far from its star, and the
        // stem would streak across the whole view as a long fine line — only
        // draw stems at a sane screen length.
        if (Math.hypot(lx - sx, ly - sy) < 170 * dpr) {
      ...
      // skip stars far outside the viewport — their long rays would otherwise
      // sweep into the visible area at close zoom
